...
const CandidatesLogin = () => {
  ...
  const { backendUrl, setUserData, setUserToken, setIsLogin, setIsAdmin } =
    ...
    try {
      ...
      if (data.success) {
        ...
        if (data.isAdmin) {
          ...
          navigate("/"); // or navigate to admin dashboard
        } else {
          setIsAdmin(false);
        ...
        <main className="flex-grow flex items-center justify-center">
          <div className="w-full max-w-md border border-gray-200 rounded-lg p-6 bg-white">
            ...
            <div className="mt-6 pt-6 border-t border-gray-200">
              <div className="flex items-center gap-2 text-xs text-gray-500 bg-blue-50 p-3 rounded">
                <ShieldCheck className="h-4 w-4 text-blue-600" />
                <span>
                  <strong>Admin Access:</strong> Use admin@jobportal.com
                </span>
              </div>
            </div>
          </div>
